...
namespace ConsoleRegisterStudent
{
    class Program
    {
        ...
        void run()
        {
            ...
            {
                WritePrompt();
                //Added a conversion string to int functionality here to make sure that the program won't crash from incorrect input
                ...
                switch (ValidateChoice(choice, firstChoice, secondChoice, thirdChoice, totalCredit))
                {
                    case -1:
                        ...
                        break;
                    case -2:
                        Console.WriteLine("You have already registered for this {0} course.", ChoiceToCourse(choice)); //Error 3: Minor spelling error, changed registerd to registered
                        break;
                    case -3:
                        Console.WriteLine("You can not register for more than 9 credit hours."); 
                        break;
                    case -4: //Error 1: Wrong case number, changed '0' to - 4
        ...
        {
            if (choice < 1 || choice > 7) //Error 5: There are only 7 courses at the moment, so this should be > 7
                return -1;
            else if (choice == firstChoice || choice == secondChoice || choice == thirdChoice) //Error 2: Changed && to ||, this should be comparing with 'or', not 'and'
                return -2;
            else if (totalCredit == 9) //Error 4: Changed from > 9 to ==9, since we need to know when the credit hours is the equivalent of 9
                return -3;
            return -4;
        }
